...
- Players can update limited profile details for their own linked characters through `public.update_player_character_profile(...)`: concept, private notes, and prepared spells.
...
`characters` currently stores sheet identity, player assignment, level/class/species/background, concept notes, private notes, combat basics, six ability scores, saving throw notes, skill notes, prepared spell loadouts, and player-editable resource state. This is sheet data only; rules automation belongs in later ruleset modules.
...
The current SQL replaces temporary anon policies with authenticated owner-based RLS. Player portal access is exposed through security-definer RPCs that return sanitized campaign data and constrain character state/profile updates to the signed-in player's linked campaign member row.
